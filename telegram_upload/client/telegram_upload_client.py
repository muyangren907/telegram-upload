...
PARALLEL_UPLOAD_BLOCKS = get_environment_integer('TELEGRAM_UPLOAD_PARALLEL_UPLOAD_BLOCKS', 10)
ALBUM_FILES = 10
RETRIES = 3
...
class TelegramUploadClient(TelegramClient):
    ...
    def send_one_file(self, entity, vif, no_bar, file: File, send_as_media: bool = False, thumb: Optional[str] = None,
                      retries=RETRIES):
        message = None
        if no_bar:
            progress = None
        ...
        try:
            ...
            message = self.send_one_file(entity, vif, no_bar, file, send_as_media, thumb, retries)
        except RPCError as e:
            if retries > 0:
                # click.echo(f'The file "{file.file_name}" could not be uploaded: {e}. Retrying...', err=True)
                message = self.send_one_file(entity, vif, no_bar, file, send_as_media, thumb, retries - 1)
            else:
                pass
